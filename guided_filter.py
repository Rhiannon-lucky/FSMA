...
def guidedfilter2d_color(guide, src, radius, eps, scale=None):
    ...
    if scale is not None:
        ...
        src = F.interpolate(src, scale_factor=1./scale, mode="nearest", recompute_scale_factor=True)
        guide = F.interpolate(guide, scale_factor=1./scale, mode="nearest", recompute_scale_factor=True)
        radius = radius // scale
    ...
    if scale is not None:
        guide = guide_sub
        mean_a = torch.stack([F.interpolate(mean_a[:, i], guide.shape[-2:], mode='bilinear', align_corners=False) for i in range(3)], 1)
        mean_b = F.interpolate(mean_b, guide.shape[-2:], mode='bilinear', align_corners=False)

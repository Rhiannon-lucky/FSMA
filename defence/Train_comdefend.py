...
if __name__ == "__main__":
    ...
    tram = transforms.Compose([
            transforms.Scale(311),
            transforms.CenterCrop(299),
            transforms.ToTensor()])

    train_data = torchvision.datasets.ImageFolder('/media/smarket/NVme02/data/ImageNet/ILSVRC2012_img_train',transform=tram)
    ...
    tr_sampler = torch.utils.data.sampler.SubsetRandomSampler(train_idx)
    val_sampler = torch.utils.data.sampler.SubsetRandomSampler(valid_idx)

    trainloader = torch.utils.data.DataLoader(train_data, batch_size=8,sampler=tr_sampler)
    testloader = torch.utils.data.DataLoader(train_data, batch_size=8,sampler=val_sampler)
